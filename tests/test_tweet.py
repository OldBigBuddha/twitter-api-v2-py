...
def test_normal_tweet(client: TwitterAPI.TwitterAPI) -> None:
    ...
    assert SAMPLE_TWEET['possibly_sensitive'] == tweet.possibly_sensitive, 'possibly_sensitive is wrong.'
    assert SAMPLE_TWEET['lang'] == tweet.lang, 'lang is wrong.'
    assert datetime.fromisoformat(SAMPLE_TWEET['created_at']) == tweet.created_at, 'created_at is wrong.'
...
def test_poll_object(client: TwitterAPI.TwitterAPI) -> None:
    ...
    tweet: Tweet.Tweet = client.get_tweet(SAMPLE_TWEET['id'], [Tweet.Expantion.POLL_IDS], poll_fields=POLL_FIELDS)
    ...
    for poll in tweet.polls:
        ...
        for idx, option in enumerate(poll.options):
            ...
            assert option.label == SAMPLE_TWEET['options'][idx]['label'], 'label is wrong.'
            assert option.votes == SAMPLE_TWEET['options'][idx]['votes'], 'votes is wrong.'

        assert poll.duration_minutes == SAMPLE_TWEET['duration_minutes'], 'duration_minutes is wrong.'
